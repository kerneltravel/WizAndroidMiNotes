...
import cn.code.notes.data.Contact;
...
public class NoteItemData {
	...
	public NoteItemData(Context context, Cursor cursor) {
		...
		mHasAttachment = (cursor.getInt(HAS_ATTACHMENT_COLUMN) > 0) ? true
				: false;
		mModifiedDate = cursor.getLong(MODIFIED_DATE_COLUMN);
		mNotesCount = cursor.getInt(NOTES_COUNT_COLUMN);
		mParentId = cursor.getLong(PARENT_ID_COLUMN);
		mSnippet = cursor.getString(SNIPPET_COLUMN);
		mSnippet = mSnippet.replace(WizGlobals.TAG_CHECKED, "").replace(
	...
	public boolean isCallRecord() {
		return (mParentId == Notes.ID_CALL_RECORD_FOLDER && !TextUtils

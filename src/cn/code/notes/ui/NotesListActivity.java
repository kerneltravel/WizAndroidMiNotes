...
import cn.code.notes.tool.BackupUtils;
import cn.code.notes.tool.DataUtils;
import cn.code.notes.tool.ResourceParser;
import cn.code.notes.ui.NotesListAdapter.AppWidgetAttribute;
import cn.code.notes.widget.NoteWidgetProvider_2x;
import cn.code.notes.widget.NoteWidgetProvider_4x;

/*ListView.MultiChoiceModeListener,*/
public class NotesListActivity extends Activity implements OnClickListener,
		OnItemLongClickListener, WizApiEvents {
	...
	private static final int MENU_NOTE_DELETE = 10;
	private static final int MENU_NOTE_VIEW = 11;

	private static final String PREFERENCE_ADD_INTRODUCTION = "cn.code.notes.introduction";
	...
	private void setAppInfoFromRawRes() {
		...
		if (!sp.getBoolean(PREFERENCE_ADD_INTRODUCTION, false)) {
			...
			WorkingNote note = WorkingNote.createEmptyNote(this,
					Notes.ID_ROOT_FOLDER,
					AppWidgetManager.INVALID_APPWIDGET_ID,
					Notes.TYPE_WIDGET_INVALIDE, ResourceParser.RED);
			note.setWorkingText(sb.toString());
			if (note.saveNote()) {
				sp.edit().putBoolean(PREFERENCE_ADD_INTRODUCTION, true)
						.commit();
			} else {
	...
	protected void onStart() {
		...
		mAccountUserId = WizSQLite.getAccountUserId(this);
	...
	private class NewNoteOnTouchListener implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN: {
				...
				/**
				 * HACKME:When click the transparent part of "New Note" button,
				 * dispatch the event to the list view behind this button. The
				 * transparent part of "New Note" button could be expressed by
				 * formula y=-0.12x+94锛圲nit:pixel锛� and the line top of the
				 * button. The coordinate based on left of the "New Note"
				 * button. The 94 represents maximum height of the transparent
				 * part. Notice that, if the background of the button changes,
				 * the formula should also change. This is very bad, just for
				 * the UI designer's strong requirement.
				 */
				if (event.getY() < (event.getX() * (-0.12) + 94)) {
					View view = mNotesListView.getChildAt(mNotesListView
							.getChildCount()
							- 1
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				WizGlobals.showMessage(
						...
						getString(R.string.format_move_notes_to_folder,
								mNotesListAdapter.getSelectedCount(), adapter
										.getFolderName(NotesListActivity.this,
												which)));
	...
	private void batchDelete() {
		new AsyncTask<Void, Void, HashSet<AppWidgetAttribute>>() {
			protected HashSet<AppWidgetAttribute> doInBackground(Void... unused) {
				HashSet<AppWidgetAttribute> widgets = mNotesListAdapter
						.getSelectedWidget();
				if (!isSyncMode()) {
	...
	private void deleteFolder(long folderId) {
		...
		ids.add(folderId);
		HashSet<AppWidgetAttribute> widgets = DataUtils.getFolderNoteWidget(
				mContentResolver, folderId);
		if (WizSQLite.isEmptyFolder(this, folderId)) {
			// if (!isSyncMode()) {
			// if not synced, delete folder directly
			DataUtils.batchDeleteNotes(mContentResolver, ids);
			// } else {
			// // in sync mode, we'll move the deleted folder into the trash
			// folder
			// DataUtils.batchMoveToFolder(this, mContentResolver, ids,
			// Notes.ID_TRASH_FOLER);
			// }
			if (widgets != null) {
				for (AppWidgetAttribute widget : widgets) {
			...
			WizGlobals.showMessage(this, "wenjianjia");
		}
	}

	void deleteCurrentNote(final NoteItemData data) {
		...
		builder.setPositiveButton(android.R.string.ok,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						WizGlobals.deleteCurrentNote(NotesListActivity.this,
								data.getId());
					}
				});
	...
	private void showCreateOrModifyFolderDialog(final boolean create) {
		...
		final Button positive = (Button) dialog
		...
		positive.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (DataUtils.checkVisibleFolderName(mContentResolver, name)) {
					WizGlobals.showMessage(NotesListActivity.this,
				...
				if (!create) {
					if (!TextUtils.isEmpty(name)) {
						...
						mContentResolver.update(Notes.CONTENT_NOTE_URI, values,
								NoteColumns.ID + "=?", new String[] { String
										.valueOf(mFocusNoteDataItem.getId()) });
					}
	...
	private final OnCreateContextMenuListener mFolderOnCreateContextMenuListener = new OnCreateContextMenuListener() {
		public void onCreateContextMenu(ContextMenu menu, View v,
			...
			if (mFocusNoteDataItem != null) {
				...
				// menu.add(0, MENU_FOLDER_CHANGE_NAME, 0,
	...
	private final OnCreateContextMenuListener mNoteOnCreateContextMenuListener = new OnCreateContextMenuListener() {
		public void onCreateContextMenu(ContextMenu menu, View v,
			...
			if (mFocusNoteDataItem != null) {
				menu.setHeaderTitle(mFocusNoteDataItem.getSnippet());
				menu.add(0, MENU_NOTE_VIEW, 0, R.string.menu_note_view);
				menu.add(0, MENU_NOTE_DELETE, 0, R.string.menu_note_delete);
			}
		}
	};
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case MENU_NOTE_VIEW:
			openNode(mFocusNoteDataItem);
			break;
		case MENU_NOTE_DELETE:
			// WizGlobals.sendToDesktop(this, this, mFocusNoteDataItem.getId());
			deleteCurrentNote(mFocusNoteDataItem);
			break;
	...
	private boolean isSyncMode() {
		return !WizGlobals.isEmptyString(SystemEditActivity
				.getSyncAccountName(this));
	}

	private void startPreferenceActivity() {
		Activity from = getParent() != null ? getParent() : this;
		Intent intent = new Intent(from, SystemEditActivity.class);
	...
	public boolean onItemLongClick(AdapterView<?> parent, View view,
		...
		if (view instanceof NotesListItem) {
			...
			if (mFocusNoteDataItem.getType() == Notes.TYPE_NOTE
				...
				mNotesListView
						.setOnCreateContextMenuListener(mNoteOnCreateContextMenuListener);
				// } else {
				// Log.e(TAG, "startActionMode fails");
				// }
			} else if (mFocusNoteDataItem.getType() == Notes.TYPE_FOLDER) {
				mNotesListView
						.setOnCreateContextMenuListener(mFolderOnCreateContextMenuListener);
			}
		}
		return false;
	}
	...
	public void onApiEnd(String userId, String actionName, boolean succeeded) {
		if (WizGlobals.isEmptyString(userId))
			return;
		if (succeeded && actionName.equals(WizGlobals.ActionNameForSync)) {
			isCancel = false;
			WizGlobals.showMessage(this, R.string.ticker_success, true);
		}
	}
	...
	public void onApiError(String userId, String actionName, int stringID,
		...
		if (actionName.equals(WizGlobals.ActionNameForSync)) {
			isCancel = false;
			WizGlobals.showMessage(this, R.string.ticker_fail, true);
		}

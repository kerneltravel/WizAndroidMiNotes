...
public class Task extends Node {
	...
	public JSONObject getCreateAction(int actionId) {
		...
		try {
			...
			throw new ActionFailureException(
					"fail to generate task-create jsonobject");
		}
	...
	public JSONObject getUpdateAction(int actionId) {
		...
		try {
			...
			e.printStackTrace();
			throw new ActionFailureException(
					"fail to generate task-update jsonobject");
		}
		...
		if (js != null) {
			try {
				...
				if (js.has(GTaskStringUtils.GTASK_JSON_LAST_MODIFIED)) {
					setLastModified(js
							.getLong(GTaskStringUtils.GTASK_JSON_LAST_MODIFIED));
				}
				...
				if (js.has(GTaskStringUtils.GTASK_JSON_DELETED)) {
					setDeleted(js
							.getBoolean(GTaskStringUtils.GTASK_JSON_DELETED));
				}

				// completed
				if (js.has(GTaskStringUtils.GTASK_JSON_COMPLETED)) {
					setCompleted(js
							.getBoolean(GTaskStringUtils.GTASK_JSON_COMPLETED));
				}
			} catch (JSONException e) {
				Log.e(TAG, e.toString());
				e.printStackTrace();
				throw new ActionFailureException(
						"fail to get task content from jsonobject");
			}
		}
	}
	...
	public JSONObject getLocalJSONFromContent() {
		...
		try {
			if (mMetaInfo == null) {
				...
				// synced task
				JSONObject note = mMetaInfo
						.getJSONObject(GTaskStringUtils.META_HEAD_NOTE);
				JSONArray dataArray = mMetaInfo
						.getJSONArray(GTaskStringUtils.META_HEAD_DATA);
	...
	public int getSyncAction(Cursor c) {
		try {
			...
			if (c.getLong(SqlNote.ID_COLUMN) != noteInfo
					.getLong(NoteColumns.ID)) {

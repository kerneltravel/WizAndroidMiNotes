...
public class ZipOutputStream extends FilterOutputStream {
	 ...
	 */
	private ZipEncoding zipEncoding = ZipEncodingHelper
			.getZipEncoding(DEFAULT_ENCODING);
	...
	protected void writeLocalFileHeader(ZipEntry ze) throws IOException {
		...
		if (createUnicodeExtraFields != UnicodeExtraFieldPolicy.NEVER) {
			...
			if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS
				...
				ze.addExtraField(new UnicodePathExtraField(ze.getName(), name
						.array(), name.arrayOffset(), name.limit()));
			}
			...
			if (comm != null && !"".equals(comm)) {
				...
				if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS
					...
					ze.addExtraField(new UnicodeCommentExtraField(comm,
							commentB.array(), commentB.arrayOffset(), commentB
									.limit()));
				}
			}
		}
		...
		writeVersionNeededToExtractAndGeneralPurposeBits(zipMethod, !encodable
				&& fallbackToUTF8);
		written += WORD;
	...
	protected byte[] getBytes(String name) throws ZipException {
		try {
			ByteBuffer b = ZipEncodingHelper.getZipEncoding(encoding).encode(
					name);
	...
	private void writeVersionNeededToExtractAndGeneralPurposeBits(
			final int zipMethod, final boolean utfFallback) throws IOException {

		// CheckStyle:MagicNumber OFF
		int versionNeededToExtract = 10;
		int generalPurposeFlag = (useUTF8Flag || utfFallback) ? UFT8_NAMES_FLAG
				: 0;
		if (zipMethod == DEFLATED && raf == null) {
	...
	public static final class UnicodeExtraFieldPolicy {
		 ...
		 */
		public static final UnicodeExtraFieldPolicy ALWAYS = new UnicodeExtraFieldPolicy(
				"always");
		/**
		 * Never create Unicode extra fields.
		 */
		public static final UnicodeExtraFieldPolicy NEVER = new UnicodeExtraFieldPolicy(
				"never");
		/**
